...
from awayout.agent_store import AgentSessionStore
from awayout.controllers import PairController
...
def cmd_start(args: argparse.Namespace, store: AgentSessionStore) -> int:
    ...
    if algorithm != "PAIR":
        return fail(
            f"Algorithm {algorithm} is reserved but not implemented in Agent Mode yet. "
            "Available: PAIR"
        )
